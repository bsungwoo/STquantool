...
def NS_Forest(adata, outdir, clusterLabelcolumnHeader = "louvain", rfTrees = 1000, 
    ...
    ## changed .X to .X.todense()
    ## Error when gene name starts with number
    ## Revised code in line 157
    
    ## Type conversion
    rfTrees = int(rfTrees)
    Median_Expression_Level = float(Median_Expression_Level)
    Genes_to_testing = int(Genes_to_testing)
    betaValue = float(betaValue)
    ...
    def fbetaTest(x, column, adata, Binary_RankedList,testArray, betaValue): # uses queries to perform F-beta testing at the betaValue set in parameters
        ...
        Subset_dataframe = pd.DataFrame(data = subset_adata.X.todense(), index = subset_adata.obs_names, columns = subset_adata.var_names)
    ...
    def ReportReturn(max_grouped_df):  # Cleaning up results to return as dataframe
        ...
        # Revised code
        sort_list = ['clusterName',"f-measure",'markerCount','NSForest_Markers','True Positive',
                    'True Negative','False Positive','False Negative']
        sort_list.extend(list(range(1,Genes_to_testing+1)))
        sort_list.extend(['index'])
        max_grouped_df = max_grouped_df[sort_list]
    ...
    for clust in ClusterList: #adata.obs.Clusters.cat.categories:    
        ...
        Subset_dataframe = pd.DataFrame(data = subset_adata.X.todense(), index = subset_adata.obs, columns = subset_adata.var_names)
        medianValues.loc[clust] = Subset_dataframe.median()
    
    medianValues.to_csv(os.path.join(outdir,'NSForest3_medianValues.csv'))
    ...
    #Write outs
    Binary_score_store_DF.to_csv(os.path.join(outdir,'NS-Forest_v3_Extended_Binary_Markers_Supplmental.csv'))
    NSForest_Results_Table_Fin.to_csv(os.path.join(outdir, 'NS-Forest_v3_Full_Results.csv'))
    ...
    Result.to_csv(os.path.join(outdir, 'NSForest_v3_Final_Result.csv'))
    ResultUnique = Result.drop_duplicates(subset=["clusterName"])
    ResultUnique.to_csv(os.path.join(outdir, 'NSForest_v3_Final_Result_Unique.csv'))
